...
Modules are introduced via the \ll{module} keyword. The general syntax
for a module declaration is as follows:
\begin{center}
  \ll{module} \textit{module-name} \textit{type-variables}(, \textit{constraint})? (\{ \textit{id-list} \})? \ll{where}\\
  def$_0$\\
  $\vdots$\\
  def$_n$\\
  \ll{end} \textit{module-name}
\end{center}
...
Now the body of the module contains an ordinary list of Sail top-level
definitions---everything is allowed. We start by defining the type of
the map, \ll{t}, and the signature of the comparison function we'll
use to compare keys.
\begin{lstlisting}
...
\ll{t} can also be called \ll{Map}. Next we'll define an insertion
function, \ll{insert}, for our map using an auxilliary helper
function, \ll{insert'}.
\begin{lstlisting}
...
contents into some other scope. For that we use the \ll{import}
statement. The structure of an import statement is as follows:
\begin{center}
  \ll{import} \textit{module-name}(\textit{type-arguments})
  (\{ \textit{bindings} \})?
  \ll{as} \textit{name}
\end{center}
...
\begin{enumerate}
\item
  The type \ll{IntMap.t},
\item
  its type constructor \ll{IntMap.Map : list((int, 'a)) -> IntMap.t('a)},
\item
  The function \ll{IntMap.compare : (int, int) -> int},
\item
  Function \ll{IntMap.insert' : forall ('a : Type). (int, 'a, list(('k, 'a))) -> list(('k, 'a))},
\item
  and \ll{IntMap.insert : forall ('a : Type). (int, 'a, IntMap.t('a)) -> IntMap.t('a)}
\end{enumerate}
